...
namespace CarBookCloud.Api.Controllers
{
    ...
    public class AboutsController : ControllerBase
    {
        ...
        public AboutsController(IMediator mediator) => _mediator = mediator;

        [HttpPost]
        public async Task<IActionResult> Create([FromBody] AboutCreateDto dto)
            => Ok(await _mediator.Send(new CreateAboutCommand(dto)));

        [HttpPut]
        public async Task<IActionResult> Update([FromBody] AboutUpdateDto dto)
            => Ok(await _mediator.Send(new UpdateAboutCommand(dto)));
        ...
        public async Task<IActionResult> GetAll()
            => Ok(await _mediator.Send(new GetAllAboutsQuery()));
    }
}
