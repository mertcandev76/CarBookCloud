...
namespace CarBookCloud.Api.Controllers
{
    ...
    public class PricingsController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] PricingCreateDto dto)
        {
            var result = await _mediator.Send(new CreatePricingCommand(dto));
            return Ok(result);
        }

        [HttpPut]
        public async Task<IActionResult> Update([FromBody] PricingUpdateDto dto)
        {
            var result = await _mediator.Send(new UpdatePricingCommand(dto));
            return Ok(result);
        }
